...
class Y2018D14(input: PuzzleInput = PuzzleInput.NULL) : GenericPuzzle2018(Day.D14, input) {
  ...
  private fun createRecipes(partTwo: Boolean): String {
    ...
    while (true) {
      ...
      if (partTwo) {
        when (target) {
          recipes.takeLast(targetLength)
            .joinToString("") -> return (recipes.size - targetLength).toString()
          recipes.takeLast(targetLength + 1)
            .dropLast(1)
            .joinToString("") -> return (recipes.size - (targetLength + 1)).toString()
        }
      } else {
