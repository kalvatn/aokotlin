...
class Y2018D09(input: PuzzleInput = PuzzleInput.NULL) : GenericPuzzle2018(Day.D09, input) {
  ...
  override suspend fun partTwo(): String {
    val (numberOfPlayers: Int, numberOfMarbles: Int) = "(\\d+) players; last marble is worth (\\d+) points".toRegex()
      .matchEntire(this.input.singleLine())!!.destructured.toList().map { it.toInt() }
