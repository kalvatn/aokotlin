  ...
  class GameOfLife(initialState: Map<Point, Char>) {
    ...
    fun step() {
      ...
      state.keys.forEach {
        ...
        if (current == '#') {
          newState[it] = if (it.adj4()
            .mapNotNull { p -> state[p] }.count { v -> v == '#' } == 1
          ) '#' else '.'
        } else {
          newState[it] = if (it.adj4()
            .mapNotNull { p -> state[p] }.count { v -> v == '#' } in listOf(1, 2)
          ) '#' else '.'
        }
      }
      minute += 1
    ...
    private fun step() {
      ...
      levels.keys.toList().forEach { level ->
        ...
        state.keys.forEach { point ->
          ...
          } else if (point in topRowPoints ||
            point in leftRowPoints ||
            point in bottomRowPoints ||
            point in rightRowPoints
          ) {
            val subState = levels[level - 1]!!
            val adjThisLevel = point.adj4().mapNotNull { p -> state[p] }
            val adjSub = when (point) {
              Point(0, 0) -> listOf(
                subState[center + Direction.NORTH.toPointDiff()], subState[center + Direction.WEST.toPointDiff()]
              )
              Point(0, 4) -> listOf(
                subState[center + Direction.SOUTH.toPointDiff()], subState[center + Direction.WEST.toPointDiff()]
              )
              Point(4, 0) -> listOf(
                subState[center + Direction.NORTH.toPointDiff()], subState[center + Direction.EAST.toPointDiff()]
              )
              Point(4, 4) -> listOf(
                subState[center + Direction.SOUTH.toPointDiff()], subState[center + Direction.EAST.toPointDiff()]
              )
